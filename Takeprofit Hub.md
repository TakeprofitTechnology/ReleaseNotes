...
* Fixed the default LmaxMTF configuration so new makers correctly use the bundled instruments file.
...
* Delayed orders execute at the worst market price observed during the delay window (TPT bridges “action W” behavior).
...
* Fixed reporting of negative extra slippage in execution logs and Trading Performance metrics. Markup P&L now correctly reflects client givebacks.
* Fixed an issue in Market Watch where High/Low values could display data from a previously selected symbol.
...
* Optimized Deals, Orders, and Slippage report generation for significantly faster downloads without blocking trading activity.

## Version 2026.6.30.408 (30 June, 2026)
### Changes
* Added support for scheduling Risk Profiles by day(s) of the week and specific date(s), in addition to the existing time window. Supports both regular and overnight schedules.
...
* Improved trading history partitioning reliability. The partitioning process now correctly handles complex position chains, ensuring historical trading data is archived successfully without interrupting normal operation.
...
* Replaced the legacy trader Open Positions page with the new SPA, providing a modernized and unified trading experience.
* Replaced the compact daily-volume tile with a prominent 30 Days Volume widget showing total volume, period-over-period change, and rolling average daily volume (ADV).
...
* Inner logic of processing database large amount of data is improved.
...
* External order data is now stored only on consumer order levels.
...
## Version 2026.01.22.689 (22 January, 2026)
### Changes
* Timeframes H4 and D1 are removed from market watch.
...
* Browser data representation is improved.
...
* Price channels tab is improved (some minor GUI fixes and sorting by status and name is added).
